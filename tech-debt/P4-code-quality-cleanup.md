...
Make stack/list right-click behavior match the visual view. Reuse the shared menu-content helper from `deck-workspace-card-action-menu-items.tsx` (wired by `deck-workspace-overflow-menus.tsx`) so dropdown/context menu actions stay in sync.

### 2. Deduplicate card action menu content

**File:** `src/app/decks/[id]/deck-workspace-card-action-menu-items.tsx` (+ `deck-workspace-overflow-menus.tsx`)

Keep commander, cover image, printing, finish, tag, and remove-card actions in the shared helper used by both `ContextMenu` and `DropdownMenu` surfaces so they cannot drift apart.
