...
export function CardPickerDialog({ open, onOpenChange, onPicked }: Props) {
  ...
  useEffect(() => {
    ...
    if (!picked.oracle_id) {
      setPrintings([])
      setPrintingId(picked.id)
      return
    }
    setLoadingPrintings(true)
    void getPrintingsByOracleId(picked.oracle_id).then(p => {
      setPrintings(p)
      setPrintingId(p[0]?.id ?? picked.id)
          ...
          <div className="space-y-3">
            ...
            <div className="max-h-72 overflow-y-auto rounded border border-border">
              {results.slice(0, 20).map(c => (
                ...
                >
                  {c.image_uris?.small && <img src={c.image_uris.small} className="w-7 rounded" />}
            ...
            <div className="grid grid-cols-3 gap-2 max-h-80 overflow-y-auto">
              {(printings.length > 0 ? printings : []).map(p => {
                ...
                return (
                  ...
                  >
                    {p.image_uris?.small && <img src={p.image_uris.small} className="w-full rounded" />}
